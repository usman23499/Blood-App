...
const submit=()=>{
  ...
  if(value.length<1 || Pass.length <1 || names.length <1 || blood.length<1 ){
...
let user={
  ...
  database().ref('/').child('blood/User/'+res).set(user)

  

  props.navigation.navigate('Home',{name:{names},email:{value}})
